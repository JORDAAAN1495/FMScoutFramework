...
namespace FM.Pitchside.Core.Entities.Ingame.Persons
{
    ...
    public class ActualPerson : BaseObject, IActualPerson
    {
        ...
        public Int64 NumberOfRelationships
        {
            get
            {
                ...
                {
                    return ProcessManager.ReadArrayLength(arrayStartAddress, 0x10);
                }
        ...
        // Virtual Methods
        public void RegisterPlayerForTeam(Team team)
        {
            // We'll need to do some weird stuff here
            // First, let's find out if we have a "registered for squad" entry
            // (Easy Case)
            bool hasSquadRegistration = false;
            foreach (Relationship rel in Relationships)
            {
                if (rel.Type == (byte)RelationshipType.RTRegisteredForSquad)
                {
                    // Found it! Found it!
                    // Just update the record
                    rel.AssociatedObjectAddress = team.MemoryAddress;
                    hasSquadRegistration = true;
                }
            }

            if (!hasSquadRegistration)
            {
                // Tough case
                // We need to move all records to a newly allocated space
                // (if there are any at all)
                // and add a new SquadRegistration relationship
                if (NumberOfRelationships > 0)
                {
                    // Let's read the whole array into our memory
                    Int64 contentStartAddress = PropertyInvoker.Get<Int64>(ActualPersonOffsets.Relationships, OriginalBytes, MemoryAddress, DatabaseMode);
                    Int64 contentEndAddress = PropertyInvoker.Get<Int64>((ActualPersonOffsets.Relationships + 0x8), OriginalBytes, MemoryAddress, DatabaseMode);
                    Int64 totalLength = contentEndAddress - contentStartAddress;

                    byte[] relationshipsBuffer = ProcessManager.ReadProcessMemory((MemoryAddress + ActualPersonOffsets.Relationships), (uint)totalLength);

                    // Now allocate relationshipsBuffer.length + 1 relationships (0x10)
                    Int64 newStartAddress = ProcessManager.AllocateProcessBytes(relationshipsBuffer.Length + 0x10);
                    // Copy the data to the new address
                    ProcessManager.WriteProcessMemory(newStartAddress, relationshipsBuffer, (uint)relationshipsBuffer.Length);
                    Int64 offset = newStartAddress + relationshipsBuffer.Length;
